 ...
 */
task("balances-avax", "Prints the list of balances for AVAX", async (args, hre) => {
  const accounts = await hre.ethers.getSigners();
  assert(accounts.length > 0, "missing accounts");
  for (const { address } of accounts) {
      ...
      if (!amount.isZero() && amount.gte(threshold)) {
        if (mint) {
          try {
            ...
            console.log(`[MINT] nonce = ${nonce.toHexString()} => ${amount} XPOW`);
          } catch (ex) {
            console.log(`[FAIL] nonce = ${nonce.toHexString()} => ${amount} XPOW`);
            console.error(ex);
            break;
          }
        } else {
          console.log(`[WORK] nonce = ${nonce.toHexString()} => ${amount} XPOW`);
        }
      }
    }
  });
